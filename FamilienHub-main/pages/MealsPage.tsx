...
const MealsPage: React.FC<MealsPageProps> = ({ 
  ...
  const generatePlan = async () => {
    ...
    if (newPlan && newPlan.length > 0) {
       const mappedPlan = newPlan.slice(0, 7).map((item, index) => {
           if (index < displayDays.length) {
               return { ...item, day: displayDays[index].dayName }; 
           }
           return item;
       });
  ...
  const renderPlan = () => (
    <div className="space-y-6 animate-fade-in">
        {!isChild && (
            <div className={`rounded-2xl p-6 shadow-lg relative overflow-hidden ${liquidGlass ? 'liquid-shimmer-card' : 'bg-gradient-to-br from-emerald-500 to-teal-600 text-white'}`}>
              <div className="flex items-center space-x-2 mb-4 relative z-10">
                <ChefHat size={28} className={liquidGlass ? 'text-emerald-700 dark:text-emerald-400' : ''} />
                <h2 className={`text-xl font-bold ${liquidGlass ? 'text-emerald-900 dark:text-emerald-100' : ''}`}>Planer</h2>
              </div>
              <div className="space-y-3 relative z-10">
                <input type="text" value={preferences} onChange={(e) => setPreferences(e.target.value)} placeholder="z.B. Vegetarisch" className={`w-full rounded-lg px-4 py-2 text-sm outline-none ${liquidGlass ? 'bg-white/50 border border-white/40 placeholder-emerald-800/50' : 'text-gray-800'}`} />
                <button onClick={generatePlan} disabled={loadingPlan} className="w-full bg-white text-emerald-600 font-bold py-2 rounded-lg shadow-sm flex items-center justify-center space-x-2 hover:bg-gray-50 active:scale-[0.98] transition disabled:opacity-70">
                  {loadingPlan ? <RefreshCcw className="animate-spin" size={18} /> : <Sparkles size={18} />}
                  <span>{loadingPlan ? 'Verarbeite...' : 'Plan erstellen'}</span>
                </button>
              </div>
            </div>
        )}
