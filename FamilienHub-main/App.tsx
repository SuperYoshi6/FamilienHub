  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[100] overflow-hidden">
      {flakes.map(flake => (
        <div 
          key={flake.id}
          className="absolute text-white/70 animate-rain will-change-transform"
          style={{
            left: `${flake.left}%`,
            top: `-20px`,
...
const App: React.FC = () => {
  ...
  const [christmasMode, setChristmasMode] = useLocalSetting<boolean>('fh_christmas', false);
  ...
  const getAppBackgroundClass = () => {
      if (christmasMode) {
          return 'bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-red-100 via-green-50 to-red-100 dark:from-red-900/40 dark:via-green-900/20 dark:to-slate-900';
      }
      if (liquidGlass) {
  ...
  return (
    <NotificationContext.Provider value={contextValue}>
      {/* Global Snow Effect Overlay */}
      {christmasMode && <SnowOverlay />}
      ...
      */}
      ...
      >
        {/* REMOVED GLOBAL HEADER TO FIX DOUBLE HEADER ISSUE */}
        {renderPage()}
        <Navigation 
